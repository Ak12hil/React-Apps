...
export default function Tasks({tasks,handleAddTask,handleDeleteTask}) {
  return (
    <section>
      ...
      <NewTask handleAddTask={handleAddTask} handleDeleteTask={handleDeleteTask}/>
      ...
      {tasks.length>0 && <ul className='p-4 mt-8 rounded-md bg-stone-100'>
        {tasks.map((task)=>(
            <li key={task.taskId} className='flex justify-between my-4'>
                <span>{task.text}</span>
                <button className='text-stone-700 hover:text-red-500' onClick={()=>handleDeleteTask(task.taskId)}>Clear</button>
            </li>
        ))}
        </ul>}
    </section>
